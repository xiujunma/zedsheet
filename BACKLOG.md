...
| | ~~**P1**~~ (partial) ✅ | Modern dynamic-array helpers: `LET`, `LAMBDA`, `MAP`, `REDUCE`, `BYROW`, `BYCOL`, `MAKEARRAY` | **Shipped 2026-06-29**: `LET` (commit `807636c`), `LAMBDA` + `MAP` (commit `00557b8`). LET pushes a name-binding frame so a body span sees the bindings; LAMBDA captures params + body tokens as a `Value::Lambda`; MAP applies the lambda element-wise via the shared `call_lambda` helper. 18 new unit tests. **REDUCE / BYROW / BYCOL / MAKEARRAY deferred** — they reuse the same `call_lambda` primitive and are short follow-ups. |
