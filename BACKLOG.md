...
| | ~~**P2**~~ ✅ | Fill-handle drag fills only one axis (down OR right, never both) | Fixed 2026-07-07 (commit `8bdd90b`): `apply_fill` previously picked whichever delta was bigger and only filled that direction, so a diagonal drag (e.g. `A1:B2 → D4`) only filled the rows OR the columns. Rewrite walks every newly-included cell in the destination rectangle and looks up its source twin via a 2-D modular offset — `src_r = sr0 + (tr - sr0) % src_h`, `src_c = sc0 + (tc - sc0) % src_w` — so the source rectangle tiles across the destination like wallpaper. Pure-down, pure-right, and diagonal drags all work in one loop. Extracted the pure text-lookup math into a new `core::data_proxy::fill_area` helper so the tiling can be host-tested. 4 new tests pin pure-down, diagonal-drag, single-cell-source, and cancelled-drag behaviors. **Trade-off**: the new path doesn't continue numeric series across rows / cols the way the old `fill_line` did for axis-aligned fills. Acceptable: Excel's fill handle only continues series along the drag axis, so the diagonal case has no good answer; the 2-D tile is the natural behaviour. |
