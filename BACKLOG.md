...
| | **P2** | Auto-save to IndexedDB (debounced) | `persist.rs` already does localStorage; IndexedDB gives room for large workbooks. **Today**: the host can wire this via the existing `on_change` callback + an IndexedDB write on their end (no built-in). A built-in debounced path would need a new `idb` / `indexed_db_futures` dep. |
