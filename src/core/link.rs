...
pub fn normalize_link(raw: &str) -> Option<String> {
    ...
    const SCHEMES: [&str; 6] = ["http://", "https://", "mailto:", "tel:", "ftp://", "ftps://"];
...
mod tests {
    ...
    fn keeps_existing_scheme() {
        assert_eq!(normalize_link("https://a.com"), Some("https://a.com".into()));
        assert_eq!(normalize_link("http://a.com/x?y=1"), Some("http://a.com/x?y=1".into()));
        assert_eq!(normalize_link("mailto:x@a.com"), Some("mailto:x@a.com".into()));
    ...
    fn bare_host_gets_https() {
        assert_eq!(normalize_link("example.com"), Some("https://example.com".into()));
        assert_eq!(normalize_link("example.com/path"), Some("https://example.com/path".into()));
        assert_eq!(normalize_link("www.example.com"), Some("https://www.example.com".into()));
    }

    #[test]
    fn email_gets_mailto() {
        assert_eq!(normalize_link("bob@example.com"), Some("mailto:bob@example.com".into()));
        assert_eq!(
    ...
    fn at_without_domain_dot_is_not_email() {
        ...
        assert_eq!(normalize_link("user@localhost"), Some("https://user@localhost".into()));
    }
}
