...
fn render_chips(
    ...
    if let Some(ci) = state.value {
        ...
        zh.push_str(&format!(
            "<div data-field=\"{}\" data-zone=\"values\" class=\"zs-pivot-chip zs-pivot-zone-chip\" \
              style=\"display:inline-flex;align-items:center;gap:4px;padding:2px 6px;margin:2px;\
              border:1px solid #ccc;border-radius:10px;background:#f7f7f7;\">\
              <span>{} of {}</span>\
              <select class=\"zs-pivot-agg\" data-field=\"{}\" style=\"font-size:11px;padding:1px;\">\
              ...
              <span class=\"zs-pivot-remove\" data-field=\"{}\" data-zone=\"values\" \
                style=\"cursor:pointer;color:#999;font-weight:bold;\">×</span>\
            </div>",
            ci,
            esc(&agg_label(state.agg)),
            esc(&label),
            ci,
            sel(state.agg, Agg::Sum),
            sel(state.agg, Agg::Count),
            sel(state.agg, Agg::Avg),
            sel(state.agg, Agg::Min),
            sel(state.agg, Agg::Max),
            ci,
        ));
    } else {
...
pub(crate) fn wire_pivot_modal(
    ...
    el.add_event_listener("click", move |event: web_sys::Event| {
        ...
        // Remove chip: return the field to "available".
    ...
    // Aggregation change (delegated `change`).
    ...
    el2.add_event_listener("change", move |event: web_sys::Event| {
        ...
        STATE.with(|s| s.borrow_mut().agg = agg);
    ...
    // Save.
    {
        ...
        el4.add_event_listener("click", move |event: web_sys::Event| {
            ...
            if state.value.is_none() {
                pivot_alert("Pick a field for Values (click Values, then a chip).");
                return;
            }
